...
var _require = __webpack_require__(0),
    Point = _require.Point; //a function to remove duplicate object in array
...
var layoutUtilities = function layoutUtilities(cy, options) {
  ...
  instance.packComponents = function (components) {
    ...
    components.forEach(function (component, index) {
      ...
      var componentPolyomino = new polyominoPacking.Polyomino(x1, y1, componentWidth, componentHeight, gridStep, index); //fill nodes to polyomino cells

      component.nodes.forEach(function (node) {
        //top left cell of a node
        var topLeftX = Math.floor((node.x - x1) / gridStep);
        var topLeftY = Math.floor((node.y - y1) / gridStep); //bottom right cell of a node

        var bottomRightX = Math.floor((node.x + node.width - x1) / gridStep);
        var bottomRightY = Math.floor((node.y + node.height - y1) / gridStep); //all cells between topleft cell and bottom right cell should be occupied
      ...
      component.edges.forEach(function (edge) {
        ...
        p1.y = (edge.endY - y1) / gridStep; //for every edge calculate the super cover 
    ...
    var mainGrid = new polyominoPacking.Grid(gridWidth * 2 + gridStep, gridHeight * 2 + gridStep, gridStep); //place first (biggest) polyomino in the center
...
(function () {
  ...
  var _require = __webpack_require__(2),
      Options = _require.Options; // registers the extension on a cytoscape lib ref
